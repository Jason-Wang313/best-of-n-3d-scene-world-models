...
2. **A theorem about all candidate-selection world models.** Rejected. A broad theorem would either be trivial proxy overoptimization or too assumption-heavy.
3. **A diagnostic for sparse-view impostor exploitation.** Chosen. It is narrow, testable, architecture-specific, and under-covered by prior work.
4. **A new repair method.** Secondary. The repair is useful but not strong enough to be the main novelty.

## Final Thesis

Sparse-view candidate selection in learned 3D scene-world models can amplify geometry errors. As the candidate count increases, a proxy scorer that only checks a few views is increasingly likely to select a candidate that matches those views while disagreeing with the target in hidden 3D regions.
...
- Sparse-view NeRF and occupancy papers repair reconstruction, but do not study sparse-view selection over sampled future scenes.
...
- Repair: ensemble-consensus and view-coverage-aware reranking raises true IoU to 0.888 at N=16 and keeps the exploitation gap far below the naive sparse-view selector.
